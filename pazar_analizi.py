...
from fiyat_parserleri import tum_kaynaklari_tara, URUN_ES_ANLAMLI
...
class FiyatKonsensusMotoru:
    ...
    def verileri_yukle(self):
        ...
        if kayitlar:
            ...
            if "fiyat_norm" in self.veri_cercevesi.columns:
                self.veri_cercevesi["fiyat_norm"] = pd.to_numeric(self.veri_cercevesi["fiyat_norm"], errors="coerce")
        else:
            self.veri_cercevesi = pd.DataFrame()
    ...
    def canli_cek(self, ilerleme_callback=None, tamamlama_callback=None):
        def calisma():
            veriler, basarili, basarisiz, detaylar = tum_kaynaklari_tara(ilerleme_callback)
            self.detaylar = detaylar
            if veriler:
                self.vt.kayit_ekle(veriler)
                self.verileri_yukle()
            if tamamlama_callback:
                tamamlama_callback(veriler, basarili, basarisiz, detaylar)
    ...
    def anomali_ve_guven_analizi(self, secili_urun):
        ...
        fiyatlar = fiyatlar[fiyatlar > 0]
        if len(fiyatlar) < 2:
            urun_verisi["Durum"] = "Doğrulandı"
            urun_verisi["Güven Skoru"] = urun_verisi.get("guven_skoru", "Tek Kaynak")
            return urun_verisi
...
class PremiumPazarAnalizi(ctk.CTk):
    ...
    def arayuz_insat_et(self):
        ...
        self.ilerleme_box.grid(row=8, column=0, padx=25, pady=(0, 10), sticky="ew")
        ...
        self.detay_box = ctk.CTkFrame(self.sidebar, fg_color=RENK_KART, border_color=RENK_BORDER, border_width=1, corner_radius=10)
        self.detay_box.grid(row=9, column=0, padx=25, pady=(0, 25), sticky="ew")
        self.detay_label = ctk.CTkLabel(self.detay_box, text="", font=ctk.CTkFont(family="Courier", size=10), text_color=RENK_YAZI_IKINCI, justify="left")
        self.detay_label.pack(padx=15, pady=10, anchor="w")
        ...
        self.kart_grid.grid(row=0, column=0, sticky="ew", pady=(0, 12))
        ...
        self.trend_grid.grid(row=1, column=0, sticky="ew", pady=(0, 12))
        ...
        self.frame_grafik.grid(row=2, column=0, sticky="nsew", pady=(0, 12))
        ...
        kolonlar = ("isletme", "kaynak_turu", "cekilen_isim", "birim", "fiyat", "fiyat_norm", "tarih", "durum", "skor")
        ...
        self.tablo.heading("fiyat_norm", text="Normalize (kg)")
        self.tablo.heading("tarih", text="Cekim Zamani")
        self.tablo.heading("durum", text="Dogrulama")
        self.tablo.heading("skor", text="Guven")

        self.tablo.column("isletme", width=160, anchor="w")
        self.tablo.column("kaynak_turu", width=90, anchor="center")
        self.tablo.column("cekilen_isim", width=200, anchor="w")
        self.tablo.column("birim", width=70, anchor="center")
        self.tablo.column("fiyat", width=80, anchor="center")
        self.tablo.column("fiyat_norm", width=90, anchor="center")
        self.tablo.column("tarih", width=120, anchor="center")
        self.tablo.column("durum", width=110, anchor="center")
        self.tablo.column("skor", width=70, anchor="center")
    ...
    def ekrani_guncelle(self, secili_urun, trend_modu=False):
        ...
        if trend_modu and not self.motor.gecmis_veri.empty:
            gun_deger = 999999 if self.gun_secici.get() == "Tum" else int(self.gun_secici.get())
            trend = self.motor.trend_analizi(secili_urun, gun=gun_deger)
            if trend:
        ...
        for _, satir in analiz_df.sort_values(by="fiyat").iterrows():
            ...
            birim_str = str(satir.get("birim", "-")) if pd.notna(satir.get("birim")) else "-"
            fnorm_str = f"{satir['fiyat_norm']:.2f} TL/kg" if pd.notna(satir.get("fiyat_norm")) and satir.get("fiyat_norm") else "-"
            guven_str = str(satir.get("Güven Skoru", "-"))
            guven_etiket = GUVEN_ETIKETLERI.get(guven_str, guven_str)
